...
//   triggers.push_back("HscpPathCaloMet");
   ...
   cout<<"hello1 7 TeV"<<endl;
#else
   keepOnlySamplesOfNameX(samples,"Data8TeV");
   cout<<"hello1 8 TeV"<<endl;
#endif
   cout<<"landauC1 = "<<landauC1<<endl;
   printf("----------------------------------------------------------------------------------------------------------------------------------------------------\n");
   ...
   chain->Add("/afs/cern.ch/user/t/telenz/PixelGain_LanGausBetterFit.root");
   //chain->Add("/afs/cern.ch/user/t/telenz/PixelAndStripGain.root");
      

   for(unsigned int s=0;s<samples.size();s++){
     samples[s].print();
     std::vector<string> FileName;
     cout<<"BaseDirectory = "<<BaseDirectory<<endl;
     ...
     for(Long64_t e=0;e<tree.size();e++){
      ...
      //if(CurrentRunIndex>199570) continue;
	 ...
	 const DeDxData* myObj  = dEdxEstimOnTheFly(tree,track,&dedxSObj,1.0, 1,0,tree.eventAuxiliary().run(),chain);
	 //cout<<"myObj1->dEdx() = "<<myObj->dEdx()<<endl;
	 ...
	 if(!PassPreselection(hscp, dedxSObj, dedxMObj, tof, dttof, csctof, tree))continue;
         //cout<<"myObj3->dEdx() = "<<myObj->dEdx()<<endl;
         for(unsigned int i=0;i<triggers.size();i++){
