...
#define FWLITE
#include "/afs/cern.ch/user/t/telenz/work/CMSSW_5_3_14_patch2/src/SUSYBSMAnalysis/HSCP/test/ICHEP_Analysis/Analysis_Global.h"
#include "/afs/cern.ch/user/t/telenz/work/CMSSW_5_3_14_patch2/src/SUSYBSMAnalysis/HSCP/test/ICHEP_Analysis/Analysis_CommonFunction.h"

#include "/afs/cern.ch/user/t/telenz/work/CMSSW_5_3_14_patch2/src/SUSYBSMAnalysis/HSCP/test/ICHEP_Analysis/Analysis_PlotFunction.h"
#include "/afs/cern.ch/user/t/telenz/work/CMSSW_5_3_14_patch2/src/SUSYBSMAnalysis/HSCP/test/ICHEP_Analysis/Analysis_Samples.h"
...
//   triggers.push_back("HscpPathCaloMet");
   ...
   // Initialize landau histograms
   landauAB = new TH1F("landauAB","landauAB",100000,0,1000);
   landauC1 = new TH1F("landauC1","landauC1",100000,0,1000);
   landauC2 = new TH1F("landauC2","landauC2",100000,0,1000);
   landauD1 = new TH1F("landauD1","landauD1",100000,0,1000);
   landauD2 = new TH1F("landauD2","landauD2",100000,0,1000);
   landauABTracks = new TH1F("landauABTracks","landauABTracks",100000,0,500);
   landauC1Tracks = new TH1F("landauC1Tracks","landauC1Tracks",100000,0,500);
   landauC2Tracks = new TH1F("landauC2Tracks","landauC2Tracks",100000,0,500);
   landauD1Tracks = new TH1F("landauD1Tracks","landauD1Tracks",100000,0,500);
   landauD2Tracks = new TH1F("landauD2Tracks","landauD2Tracks",100000,0,500);
   landauABModule = new TH2F("landauABModule","landauABModule",1440,0,1440,100000,0,500);
   landauC1Module = new TH2F("landauC1Module","landauC1Module",1440,0,1440,100000,0,500);
   landauC2Module = new TH2F("landauC2Module","landauC2Module",1440,0,1440,100000,0,500);
   landauD1Module = new TH2F("landauD1Module","landauD1Module",1440,0,1440,100000,0,500);
   landauD2Module = new TH2F("landauD2Module","landauD2Module",1440,0,1440,100000,0,500);



   nHitsDetId = new TH1F("nHitsDetId","nHitsDetId",50*pow(10,6),300*pow(10,6),350*pow(10,6));
   ...
   // Open tree for pixel Calibration
   TChain* chain = new TChain("SiStripCalib/APVGain");
   chain->Add("Data8TeVGains.root");

   int   DetIdPixel[16588];
   unsigned int   Index[16588];
   Double_t Gain[16588];
   UChar_t SubDet[16588];
   unsigned int lastRun;
   unsigned int firstRun;


   chain->SetBranchAddress("DetId",DetIdPixel);
   chain->SetBranchAddress("Index",Index);
   chain->SetBranchAddress("GainWoGlobalScaling",Gain);
   chain->SetBranchAddress("SubDet",SubDet);
   chain->SetBranchAddress("firstRun",&firstRun);
   chain->SetBranchAddress("lastRun",&lastRun);
   ...
   for(int n=0; n<chain->GetEntries(); n++){
     ...
     lowerBound[n]=firstRun;
     higherBound[n]=lastRun;

     cout<<"lowerBound["<<n<<"] = "<<lowerBound[n]<<endl;
     cout<<"higherBound["<<n<<"] = "<<higherBound[n]<<endl;

     for(int j=0; j<16588; j++){

       if((int)SubDet[j]>2) continue;

       //cout<<"Gain[j] = "<<Gain[j]<<endl;
       //cout<<"DetIdPixel[j] = "<<DetIdPixel[j]<<endl;
       pixelGain[std::make_pair(n,DetIdPixel[j])] = Gain[j];
   ...
   ppixelMap=&pixelGain;
   cout<<"pixelGain.size() = "<<pixelGain.size()<<endl;
   ...
   for(unsigned int s=0;s<samples.size();s++){
     ...
     for(int name=0; name<FileName.size(); name++){
     ...
     //for(Long64_t e=0;e<10000;e++){
      ...
      //cout<<"CurrentRunIndex = "<<CurrentRunIndex<<endl;
      //cout<<"tree.eventAuxiliary().run() = "<<tree.eventAuxiliary().run()<<endl;
	 ...
	 const DeDxData* myObj  = dEdxEstimOnTheFly(tree,track,&dedxSObj,1.0, 1,0,tree.eventAuxiliary().run());
	 ...
	 //	 if(!PassPreselection(hscp, dedxSObj, *myObj, tof, dttof, csctof, tree))continue;
	    ...
	    */
	    ...
	    if(PassPreselection(hscp, dedxSObj, dedxMPObj, tof, dttof, csctof, tree))  dEdxMPProf[i]->Fill(CurrentRunIndex, dedxMPObj.dEdx());
	    if(PassPreselection(hscp, dedxSObj, *myObj, tof, dttof, csctof, tree))     myHist[i]->Fill(CurrentRunIndex, myObj->dEdx());
   ...
   c1 = new TCanvas("c1","c1",600,600);
   cout<<"dEdxMPProf[i]->GetMean(1) = "<<dEdxMPProf[i]->GetMean(1)<<endl;
   ...
   delete c1;
   TString auxString1=(TString) "pictures/" + triggers[i] + (TString) "Profile_ImP.root";
   TFile f1(auxString1,"new");
   myHist[i]->Write();
   f1.Close();


   c1 = new TCanvas("c1","c1",600,600);
   myHist[i]->LabelsDeflate("X");
   myHist[i]->LabelsOption("av","X");
   myHist[i]->GetXaxis()->SetNdivisions(505);
   myHist[i]->SetTitle("");
   myHist[i]->SetStats(kFALSE);
   myHist[i]->GetXaxis()->SetTitle("");
   myHist[i]->GetYaxis()->SetTitle("dE/dx estimator");
   myHist[i]->GetYaxis()->SetTitleOffset(0.9);
   myHist[i]->GetXaxis()->SetLabelSize(0.04);
   myHist[i]->SetLineColor(Color[0]);
   myHist[i]->SetFillColor(Color[0]);
   myHist[i]->SetMarkerSize(0.4);
   myHist[i]->SetMarkerStyle(Marker[0]);
   myHist[i]->SetMarkerColor(Color[0]);
   myHist[i]->Draw("E1");
   c1->Modified();
   c1->SetGridx(true);
   DrawPreliminary(SQRTS, IntegratedLuminosity);
   ...
   landauD1->Write();
   landauD2->Write();
   landauABTracks->Write();
   landauC1Tracks->Write();
   landauC2Tracks->Write();
   landauD1Tracks->Write();
   landauD2Tracks->Write();
   landauABModule->Write();
   landauC1Module->Write();
   ...
   TString auxString2=(TString) "pictures/" + triggers[i] + (TString) "Profile_ImP_myHist.root";
   TFile f2(auxString2,"new");
   myHist[i]->Write();
   f2.Close();
   ...
   cout<<"dEdxMPCProf[i]->GetEntries() = "<<dEdxMPCProf[i]->GetEntries()<<endl;
   c1->Modified();
   c1->SetGridx(true);
   DrawPreliminary(SQRTS, IntegratedLuminosity);
   cout<<"triggers[i] = "<<triggers[i]<<endl;
