...
TH1F* landauABafterPreSelec;
TH1F* landauC1afterPreSelec;
TH1F* landauC2afterPreSelec;
TH1F* landauD1afterPreSelec;
TH1F* landauD2afterPreSelec;
TH1F* landauABTracksafterPreSelec;
TH1F* landauC1TracksafterPreSelec;
TH1F* landauC2TracksafterPreSelec;
TH1F* landauD1TracksafterPreSelec;
TH1F* landauD2TracksafterPreSelec;
TH2F* landauABModuleafterPreSelec;
TH2F* landauC1ModuleafterPreSelec;
TH2F* landauC2ModuleafterPreSelec;
TH2F* landauD1ModuleafterPreSelec;
TH2F* landauD2ModuleafterPreSelec;


std::map<std::pair<int,int>, double> pixelGain;
std::map<std::pair<int,int>, double> *ppixelMap;
int lowerBound[5];
int higherBound[5];
std::map<int,double> gainPixel1;
std::map<int,double> gainPixel2;
std::map<int,double> gainPixel3;
std::map<int,double> gainPixel4;
std::map<int,double> gainPixel5;

TH1F* nHitsDetId;// = new TH1F("nHitsDetId","nHitsDetId",50*pow(10,6),300*pow(10,6),350*pow(10,6));

reco::DeDxData* dEdxEstimOnTheFly(const fwlite::ChainEvent& ev, const reco::TrackRef& track, const reco::DeDxData* dedxSObj, double scaleFactor=1.0, bool usePixel=false, bool useClusterCleaning=true, int CurrentRun=0){
     fwlite::Handle<HSCPDeDxInfoValueMap> dEdxHitsH;
     dEdxHitsH.getByLabel(ev, "dedxHitInfo");
     if(!dEdxHitsH.isValid()){printf("Invalid dEdxHitInfo\n");return NULL;}
     const ValueMap<HSCPDeDxInfo>& dEdxHitMap = *dEdxHitsH.product();
	...
	if(detid.subdetId()>2) continue; // skip strip
	if(useClusterCleaning && !hscpHitsInfo.shapetest[h])continue;
	double Norm = (detid.subdetId()<3)?3.61e-06:3.61e-06*265;
        if(detid.subdetId()>2) Norm*=10.0; //mm --> cm

	double runRange=-1;

	for(int n=0; n<5; n++){

	  if(CurrentRun<=higherBound[n] && CurrentRun>=lowerBound[n]) runRange=n; 


	}
	//	cout<<"runRange = "<<runRange<<endl;
	//double gain=1;
	double gain=(*ppixelMap)[std::make_pair(runRange,detid.rawId())];
	//cout<<"gain = "<<gain<<endl;
	Norm*=1./gain;
	...
	if(usePixel && detid.subdetId()<3){
	  ...
	  for(int n=0; n<chain->GetEntries(); n++){
	    ...
	    //cout<<"lastRun = "<<lastRun<<endl;
	     if(CurrentRun<=lastRun && CurrentRun>=firstRun)
	      {
		//cout<<"If condition fulfilled"<<endl;
		for(int j=0; j<1440; j++){
		  if(detid.rawId()==DetIdPixel[j]){
		    //cout<<"detid found"<<endl;
		    ind=Index[j];
		    Norm*=1./Gain[j];
		    break;
		  }
		}
		// Fill dE/dx values in histogram for all modules and different steps 
		if(n==0){
		  landauAB->Fill(scaleFactor*Norm*hscpHitsInfo.charge[h]/hscpHitsInfo.pathlength[h]);
		  landauABModule->Fill(ind,scaleFactor*Norm*hscpHitsInfo.charge[h]/hscpHitsInfo.pathlength[h]);
		}
		else if(n==1){
		  landauC1->Fill(scaleFactor*Norm*hscpHitsInfo.charge[h]/hscpHitsInfo.pathlength[h]);
		  landauC1Module->Fill(ind,scaleFactor*Norm*hscpHitsInfo.charge[h]/hscpHitsInfo.pathlength[h]);
		}
		else if(n==2){
		  landauC2->Fill(scaleFactor*Norm*hscpHitsInfo.charge[h]/hscpHitsInfo.pathlength[h]);
		  landauC2Module->Fill(ind,scaleFactor*Norm*hscpHitsInfo.charge[h]/hscpHitsInfo.pathlength[h]);
		}
		else if(n==3){
		  landauD1->Fill(scaleFactor*Norm*hscpHitsInfo.charge[h]/hscpHitsInfo.pathlength[h]);
		  landauD1Module->Fill(ind,scaleFactor*Norm*hscpHitsInfo.charge[h]/hscpHitsInfo.pathlength[h]);
		}
		else if(n==4){
		  landauD2->Fill(scaleFactor*Norm*hscpHitsInfo.charge[h]/hscpHitsInfo.pathlength[h]);
		  landauD2Module->Fill(ind,scaleFactor*Norm*hscpHitsInfo.charge[h]/hscpHitsInfo.pathlength[h]);
		}
		else{
		  cout<<"n = "<<n<<endl;
		...
		step=n;
		//cout<<"step = "<<step<<endl; 
		nHitsDetId->Fill(detid.rawId());
		break;
	      }
	    else{
	      //cout<<"not found in "<<n<<endl; 
	      count+=1;
	    }
	  }
	  //if(count==2) cout<<"no run range found"<<endl;
	}
	*/
...
//     printf(" : %f\n",result);
     /*
     if(step==0)      landauABTracks->Fill(result);
     else if(step==1) landauC1Tracks->Fill(result);
     else if(step==2) landauC2Tracks->Fill(result);
     else if(step==3) landauD1Tracks->Fill(result);
     else if(step==4) landauD2Tracks->Fill(result);
     else{
       //cout<<"step = "<<step<<endl;
       if(size!=0){
	 cout<<"CurrentRun = "<<CurrentRun<<endl;
